...
TEST_CASE("Test bitset construction empty string", "[bitset]") {
    std::string s("");
    Bitset b(s);
    REQUIRE_FALSE(b.good());
}
...
TEST_CASE("Test reset with construction string", "[bitset]") {
    std::string s("11111");
    Bitset b(s);
    b.reset(0);
    b.reset(3);
...
TEST_CASE("Test toggle with construction string", "[bitset]") {
    std::string s("00100");
    Bitset b(s);
    b.toggle(0);
    b.toggle(2);
    REQUIRE(b.good());
    REQUIRE(b.asString().compare("10000") == 0);
}

TEST_CASE("Test test method with construction string", "[bitset]") {
    std::string s("00100");
    Bitset b(s);
    REQUIRE(b.size() == 5);
    REQUIRE(b.good());
